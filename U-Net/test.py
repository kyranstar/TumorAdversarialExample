...
def test(config_file, adversarial):
    ...
    if adversarial:
        ...
        dataloader.data = add_gaussian(dataloader.data, 0.1)
        #if(config_net1):
        #    dataloader.data = make_fgm(sess, fgm(net1, x1, eps=0.01, epochs=2, sign=True, clip_min=0., clip_max=1.), x1,
        #        dataloader.data)
        #else:
        #    dataloader.data = make_fgm(sess, fgm(net1ax, x1ax, eps=0.01, epochs=2, sign=True, clip_min=0., clip_max=1.), x1ax,
        #        dataloader.data)


        # TODO adversarial for later cascading models?

    # 5, start to test
    test_slice_direction = config_test.get('test_slice_direction', 'all')
    save_folder = config_data['save_folder']
    if adversarial:
        save_folder += "-adversarial"
...
def fgm(model, x, eps=0.01, epochs=1, sign=True, clip_min=0., clip_max=1.):
    ...
    ybar = model(xadv, is_training=False)
...
def add_gaussian(data, sd):
    def add_gaus(vol):
        return vol + np.random.normal(0.0, sd, size=vol.shape)

    return [[add_gaus(volume) for volume in volume_list] for volume_list in data]

def make_fgm(sess, fgm, x1, X_data, epochs=1, eps=0.01, batch_size=128):
    """
    Generate FGSM by running fgm.
    """
    ...
    for data in X_data:
        ...
        volume_probability_prediction(temp_imgs, data_shape, label_shape, data_channel,
                                          class_num, batch_size, sess, proby, x)

        adv = sess.run(fgm, feed_dict={
            x1: data})
        X_adv.append(adv)
...
if __name__ == '__main__':
    ...
    parser.add_argument("config_file", help="test cases config file")
    parser.add_argument("--adversarial", help="whether to use adversarial examples", action="store_true")
    args = parser.parse_args()

    config_file = args.config_file
    assert(os.path.isfile(config_file))
    test(config_file, args.adversarial)
