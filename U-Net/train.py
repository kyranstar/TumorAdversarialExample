...
from util.RandForest import RandForest
...
def extract_features(image):
    """
    Extracts haralick features for random forest
    """
...
def train_randforest(config_file):
    # 1, load configuration parameters
    config = parse_config(config_file)
    config_data  = config['data']
    config_net   = config['network']
...
if __name__ == '__main__':
    ...
    parser.add_argument("config_file", help="test cases config file. example: python train.py config17/train_wt_ax.txt")
    args = parser.parse_args()
    assert(os.path.isfile(args.config_file))
    if config_train.get('net_type') == 'RandForest':
        print('Not Implemented')
        exit()
        train_randforest(args.config_file)
    else:
