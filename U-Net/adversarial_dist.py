...
from attacks import FastGradientMethod, ProjectedGradientDescent, MomentumIterativeMethod, Noise
import matplotlib
matplotlib.use('Agg')
...
def plot(losses):
    ...
    # Create the boxplot
    bp = ax.hist(losses, density=True, bins=40)
    ax.set_xlabel('Loss')
    ax.set_ylabel('Prob')
    ax.set_title('Loss Distribution for Uniform Noise (n=%d)' % (len(losses)))
    # Save the figure
