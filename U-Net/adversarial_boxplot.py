...
def plot(loss_dists, eps_vals, title):
    # Create a figure instance
    fig = plt.figure(1, figsize=(9, 6))
    # Create an axes instance
    ax = fig.add_subplot(111)
    # Create the boxplot
    bp = ax.boxplot(loss_dists)
    ax.set_xticklabels(["{:.2f}".format(eps) for eps in eps_vals])
    ax.set_xlabel('Eps')
    ax.set_ylabel('Loss')
    ax.set_title(title)
    # Save the figure
...
if __name__ == '__main__':
    ...
    #loss_dists = [[0.1, 0.2, 0.05, 0.15], [0.35, 0.3, 0.6, 0.2], [0.3, 0.3, 0.3, 0.4], [0.1, 0.5, 0.7, 0.6]]
    #eps_vals = [0.1, 0.2, 0.3, 0.5]
    #plot(loss_dists, eps_vals)
